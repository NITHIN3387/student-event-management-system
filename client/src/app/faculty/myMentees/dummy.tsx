...
interface PropsType {
  ...
  onDelete: (sid: string | number) => Promise<void>; // Add onDelete prop
}
...
const EditableTable: FC<PropsType> = ({
  ...
  const onClickUpdate = (sid: string | number) => {
    setDisableInput(-1);
    onDelete(sid); // Call onDelete instead of onSubmit for deletion
  };
              ...
              <td key={item} className="text-left py-2 px-4 border">
                <input
                  ...
                  className={`w-full border ${
                    editableCols[i] && disableInput !== index
                      ? "border-transparent bg-white"
                      : "border-slate-400"
                  } rounded-sm px-1`}
                  disabled={editableCols[i] && disableInput !== index}
